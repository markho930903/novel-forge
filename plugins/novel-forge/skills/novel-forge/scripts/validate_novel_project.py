...
def validate_graph(project: Path, errors: list[str]) -> None:
    graph_json = project / "06_图谱索引.json"
    ...
    if graph_json.is_file():
        try:
            ...
            if not isinstance(nodes, list) or not isinstance(edges, list):
                errors.append("图谱 JSON 的 nodes/edges 必须是数组")
            else:
            ...
            errors.append(f"图谱 JSON 无法读取: {exc}")
    if graph_md.is_file():
        graph_content = graph_md.read_text(encoding="utf-8")
        if "```mermaid" not in graph_content:
            errors.append("图谱 Markdown 缺少 Mermaid 可视图")
        if "## 节点表" not in graph_content or "## 关系表" not in graph_content:
            errors.append("图谱 Markdown 缺少节点表或关系表")
